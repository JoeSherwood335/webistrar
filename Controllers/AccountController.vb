...
Namespace Controllers
    ...
    Public Class AccountController
        ...
        Private _roleservice As AccountRoleService
        Sub New()
        ...
        Sub New(ByVal formsAuth As IFormsAuthentication, ByVal service As IMembershipService)
            ...
            _roleservice = New AccountRoleService
        End Sub
        ...
        Function LogOn(ByVal userName As String, ByVal password As String, ByVal rememberMe As Boolean, ByVal returnUrl As String) As ActionResult
            ...
            Dim u = Myapp.GetUserInfo(userName)

            _roleservice.SetRolestoUserByProfile(userName, u.Profile.Name)

            If Not String.IsNullOrEmpty(returnUrl) Then
                Return Redirect(returnUrl)
            Else
                Return RedirectToAction("InProgress", "Services")
            End If
...
#End Region
    ...
    Public Class AccountRoleService
        ...
        Private _provider As RoleProvider

        Sub New()
            Me.New(Nothing)
        End Sub

        Sub New(ByVal provider As RoleProvider)
            _provider = If(provider, Roles.Provider)


        End Sub

        Function SetRolestoUserByProfile(ByVal userName As String, ByVal Profile As String) As Boolean

            Dim r As New Repository

            SetRolestoUserByProfile(userName, r.getProfile(Profile))

        End Function
        Function SetRolestoUserByProfile(ByVal userName As String, ByVal Profile As Model.profile) As Boolean

            Dim dc As New R2kdoiMVCDataContext

            Dim p = From a In Profile.ProfileGroupContainers _
                    Select a.Group.Groupname

            Dim np = From a In dc.Groups _
                     Where Not p.Contains(a.Groupname) _
                     Select a.Groupname


            For Each s In p
                Dim userlist() As String = userName.Split(",")
                Dim rolelist() As String = s.Split(",")

                If Not _provider.IsUserInRole(userName, s) Then
                    _provider.AddUsersToRoles(userlist, rolelist)
            ...
            For Each s In np
                ...
                If _provider.IsUserInRole(userName, s) Then
                    _provider.RemoveUsersFromRoles(userlist, rolelist)
